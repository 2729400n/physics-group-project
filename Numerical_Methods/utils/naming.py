...
def slugify(text: str = None, max_width: int = 32, exempt='-'):
    if text is None or text == '':
        ...
        text = ''.join(_SuperCrptoRandGen.choices(
            'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-', k=width))
...
def slugify_tk(text: str = None, max_width: int = 32, exempt='-'):
    if text is None or text == '':
        width = random.randint(4, max_width)
        text = ''.join(_SuperCrptoRandGen.choices(
            'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-', k=width))
    ...
    out = ''
    for i in range(min(len(text), max_width)):
        ch= text[i] if text[i].isalnum() or (text[i] in exempt) else '_'
        if ch.isupper():
            out+=f'_capital_{ch.lower()}'
        else:
            out += ch
    return out