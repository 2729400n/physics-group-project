...
def findUandV(grid: np.ndarray, dx: float = 1.0, dy: float = 1.0):
    ...
    E_field[1:-1, :, 1] = -(grid[2:, :] - grid[:-2, :]) / (2 * dx)
    E_field[0, :, 1] = -(grid[1, :] - grid[0, :]) / dx  # Forward difference at first row
    E_field[-1, :, 1] = -(grid[-1, :] - grid[-2, :]) / dx  # Backward difference at last row

    # Compute Ey = -dΦ/dy using central differences
    E_field[:, 1:-1, 0] = -(grid[:, 2:] - grid[:, :-2]) / (2 * dy)
    E_field[:, 0, 0] = -(grid[:, 1] - grid[:, 0]) / dy  # Forward difference at first column
    E_field[:, -1, 0] = -(grid[:, -1] - grid[:, -2]) / dy  # Backward difference at last column

    return E_field


def findUandV_ind1(grid: np.ndarray, dx: float = 1.0, dy: float = 1.0):
    """Compute the electric field components (-∇Φ) from a given potential grid."""
    

    # Return as a combined array
    return -np.array(np.gradient(grid, dy, dx))
