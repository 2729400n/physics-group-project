...
class InspectFrame(Frame):
    ...
    def __init__(self, parent, Grid_obj, dx: float, dy: float, title='',
                 xlabel='', ylabel='', figsize=(8, 8), dpi=64, *args,
                 **frame_kwargs):
        ...
        self.calculatePolyNomialButton = Button(
            button_frame, text="Polynomial Interpolate",
            command=self.calculatePolyNomial)

        self.calculatePolyNomialButton.pack()

        self.calculateLaplacian = Button(
            button_frame, text="Calulate Laplacian",
            command=self.findLaplacian)
    ...
    def select_Smallgrid(self, evt):
        ...
        self.zoomedArea = data[y_start_idx:y_end_idx + 1,
    ...
    def showAndOfferSavePolyNomial(self, message, **data):
        def __inner_func() -> None:
            ...
            dsimple = simple_diag.SimpleDialog(
                title='Polynomial Interpolation', text=message, buttons=['OK'],
                master=self, cancel=0, default=0)
            dsimple.go()
            ...
            shouldSave = simple_diag.SimpleDialog(
                ...
                title='Save or Not', buttons=['Save', "Don't Save"],
                default=1, cancel=1)
            ...
            if opt == 0:
                try:
                    ...
                    opt = simple_diag.SimpleDialog(
                        master=self, text="Failed to save Polynomial ",
                        title='Error')
            return
        return __inner_func
    ...
    def callInterpolate(self):
        try:
            c, xopt, yopt, XYOpt, *funcs = errors.InterpolateGrid_fastest(
                self.Grid_obj, self.Xs[0], self.Ys[0], self.Xs[1], self.Ys[1],
            ...
            while True:
                try:
                    self.jobqueue.put_nowait(self.showAndOfferSavePolyNomial(
                        message, xopt=xopt, yopt=yopt, XYOpt=XYOpt, Xs=self.Xs,
                        Ys=self.Ys))
                    break
                except Full:
                    pass
    ...
    def callInterpolateSubView(self):
        try:
            c, xopt, yopt, XYOpt, *funcs = errors.InterpolateGrid_fastest(
                self.zoomedArea, self.zoomedXs[0], self.zoomedYs[0],
                self.zoomedXs[1], self.zoomedYs[1], self.dy, self.dx,
                savefunc=True, Xs=self.zoomedXs, Ys=self.zoomedYs)
            ...
            while True:
                try:
                    self.jobqueue.put_nowait(self.showAndOfferSavePolyNomial(
                        message, xopt=xopt, yopt=yopt, XYOpt=XYOpt,
                        Xs=self.zoomedXs, Ys=self.zoomedYs))
                    break
                except Full:
                    pass
    ...
    def callInterpolateSplines(self, m, n):
        ...
        try:
            ...
            print(subviews_real)
            c, xopt, yopt, XYOpt, *funcs = errors.InterpolateGrid_fastest(
                self.Grid_obj, self.Xs[0], self.Ys[0], self.Xs[1],
                self.Ys[1], self.dy, self.dx, savefunc=True, Xs=self.Xs,
                Ys=self.Ys)
            ...
            while True:
                try:
                    self.jobqueue.put_nowait(self.showAndOfferSavePolyNomial(
                        message, xopt=xopt, yopt=yopt, XYOpt=XYOpt, Xs=self.Xs, Ys=self.Ys))
                    break
                except Full:
                    pass
        finally:
    ...
    def calculatePolyNomial(self):
        if (self.jobRunning):
            msgbox.showwarning(
                message="You Must Wait till the last job Finishes Before creating a new one", title="Warning")
            return
        diag = simple_diag.SimpleDialog(self, "Would you like to calulate polyNomial\nOn The Full Grid or zoomed subsection?", [
                                        "FullGrid", "Subsection", "Cancel"], 1, 2, "Error PolyNomial")
        opt = diag.go()
        self.jobRunning = True
        match opt:
            case 0:
                self.job = Thread(target=self.callInterpolate,
                                  name='Full Grid Interpolate')
                self.job.start()
            case 1:
                self.job = Thread(
    ...
    def findLaplacian(self):
        ...
        error, abserr = errors.laplaceify(
            grid=grd, dx=self.dx, dy=self.dy, wrap=lastfield[0], wrap_direction=lastfield[1])

        absroot = tk.Toplevel(self)
        errroot = tk.Toplevel(self)

        absframe = InspectFrame(absroot, abserr, self.dx, self.dy,
                                title='ABS residuals Laplacian', xlabel='X label', ylabel='Y label')

        absframe.pack()

        errframe = InspectFrame(errroot, error, self.dx, self.dy,
                                title='ABS residuals Laplacian', xlabel='X label', ylabel='Y label')
        errframe.pack()
