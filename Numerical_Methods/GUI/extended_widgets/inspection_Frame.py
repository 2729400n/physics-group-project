...
class InspectFrame(Frame):
    ...
    def __init__(self,parent,Grid_obj,dx:float,dy:float,title='',xlabel='',ylabel='',figsize=(8,8),dpi=64,*args,**frame_kwargs):
    ...
    def showAndOfferSavePolyNomial(self,message,**data):
        def __inner_func() -> None:
            ...
            dsimple =simple_diag.SimpleDialog(title='Polynomial Interpolation', text=message,buttons=['OK'],master=self,cancel=0,default=0)
            dsimple.go()
            dsimple.wm_delete_window()
            shouldSave =simple_diag.SimpleDialog(master=self,text="Would you like to save the polynomial to disk",title='Save or Not',buttons=['Save',"Don't Save"],default=1,cancel=1)
            opt = shouldSave.go()
            dsimple.wm_delete_window()
            if opt==0:
                try:
                    fname =asksaveasfilename(parent=self,defaultextension='.polynomial')
                    if fname is None:
                        raise Exception('No Name Given')
                    numpy_io.saveArrays(fname,karray=data)
                except:
                    opt=simple_diag.SimpleDialog(master=self,text="Failed to save Polynomial ",title='Error')
            return
        return __inner_func
    ...
    def callInterpolateSubView(self):
        try:
            c,xopt,yopt,XYOpt,*funcs =errors.InterpolateGrid_fastest(self.zoomedArea,self.zoomedXs[0],self.zoomedYs[0],self.zoomedXs[1],self.zoomedYs[1],
                                                self.dy,self.dx, savefunc=True,Xs=self.zoomedXs,Ys=self.zoomedYs)
            region_text = f'Region = ({self.zoomedXs[0],self.zoomedYs[0]})=>({self.zoomedXs[1],self.zoomedYs[1]})'
            message = f'c={c}\nxPoly={xopt}\nYpoly={yopt}\nXYPoly={XYOpt}\n{region_text}'

            # Schedule the message box on the main thread
            while True:
                try:
                    self.jobqueue.put_nowait(self.showAndOfferSavePolyNomial(message,xopt=xopt,yopt=yopt,XYOpt=XYOpt,Xs=self.zoomedXs,Ys=self.zoomedYs) )
                    break
                except Full:
                    pass
    ...
    def calculatePolyNomial(self):
        if(self.jobRunning):
            ...
            return 
        diag=simple_diag.SimpleDialog(self,"Would you like to calulate polyNomial\nOn The Full Grid or zoomed subsection?",["FullGrid","Subsection","Cancel"],1,2,"Error PolyNomial")
        opt = diag.go()
        self.jobRunning=True
        match opt:
            case 0:
                self.job = Thread(target=self.callInterpolate,name='Full Grid Interpolate')
                self.job.start()
            case 1:
                self.job = Thread(target=self.callInterpolateSubView,name='SubView Interpolate')
                self.job.start()
            case _ :
                self.jobRunning=False
