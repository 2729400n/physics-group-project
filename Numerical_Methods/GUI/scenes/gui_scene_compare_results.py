...
def scale_to_fit(a1, a2, copya1=True, copya2=True):
    ...
    if f1.shape != f2.shape:
        ...
        wantedShapef1 = [
            f1.shape[int(i // 2)] if ((i % 2) == 0) else 1 for i in range(2 * f1.ndim)
        ]
        wantedShapef2 = [
            f2.shape[int(i // 2)] if ((i % 2) == 0) else 1 for i in range(2 * f2.ndim)
        ]

        f1slices = [
            slice(0, wantedShapef1[i], 1) if ((i % 2) == 0) else None
            for i in range(2 * f1.ndim)
        ]
        f2slices = [
            slice(0, wantedShapef2[i], 1) if ((i % 2) == 0) else None
            for i in range(2 * f2.ndim)
        ]
        ...
        f1 = np.broadcast_to(f1[*f1slices],
                             wantedShapef1).reshape(tuple(finalshape1))
        
        f2 = np.broadcast_to(f2[*f2slices],
                             wantedShapef2).reshape(tuple(finalshape2))

        del finalshape1, finalshape2, f1slices, f2slices, wantedShapef1
        del wantedShapef2
                            ...
                            newindex += [
                                (
                                    tuple(
                                        int(j // 2)
                                        for j in range(
                                            2 * int(np.ceil(finalslice1[i] / 2))
                                        )
                                    )
                                    + tuple(range(f1.shape[k]))[
                                        int(np.ceil(finalslice1[i] / 2)) : (
                                            (int(finalslice1[i] // 2) // 2) + 1
                                        )
                                    ]
                                    + tuple(
                                        f1.shape[i] - (int(j // 2) + 1)
                                        for j in range(2 * int(finalslice1[i] // 2))
                                    )
                                ),
                                None,
                            ]
                #                print('f1',newindex)
                f1 = f1[*newindex].reshape(
                    tuple(
                        f1.shape[z] if z != i else (f1.shape[i] + finalslice1[i])
                        for z in range(len(f1.shape))
                    )
                )
        ...
        for i in range(len(finalslice2)):
            if finalslice2[i] != 0:
                ...
                for z in range(2 * len(f2.shape)):
                    if (z % 2) == 0:
                        ...
                        if k != i:
                            ...
                            newindex += [
                                (
                                    tuple(int(j // 2) for j in range(finalslice2[i]))
                                    + tuple(range(f2.shape[k]))[
                                        int(finalslice2[i] // 2) :
                                    ]
                                    + tuple(int(j // 2) for j in range(finalslice2[i]))
                                ),
                ...
                f2 = f2[*newindex].reshape(
                    tuple(
                        f2.shape[z] if z != i else (f2.shape[i] + finalslice2[i])
                        for z in range(len(f2.shape))
                    )
                )
...
def scale_to_scale(a1, a2, copya1=True, copya2=True):
    ...
    if f1.shape != f2.shape:
        ...
        for i in range(f1.ndim):
            ...
            elif f1.shape[i] > f2.shape[i]:
                # Expand f2 by repeating cyclically
                repeats = f1.shape[i] // f2.shape[i]  # Full repeats
                remainder = f1.shape[i] % f2.shape[i]  # Leftover elements
                # wantedShapef2[i*2]=f1.shape[i]
                n = repeats + (1 if remainder else 0)
    ...
    if f1.shape != f2.shape:
        ...
        for i in range(f1.ndim):
            ...
            if f1.shape[i] < f2.shape[i]:  # Expand f1 by repeating cyclically
                ...
                remainder = f2.shape[i] % f1.shape[i]  # Leftover elements
                f1 = np.tile(
                    f1, (repeats + (1 if remainder else 0),) + (1,) * (f1.ndim - i - 1)
                )
                ...
                f2 = np.tile(
                    f2, (repeats + (1 if remainder else 0),) + (1,) * (f2.ndim - i - 1)
                )
...
class CompareScene(ttk.Frame):
    ...
    def __init__(self, master=None, *args, **kwargs):
        ...
        self.treeFrame.pack(
            fill=tk.BOTH, anchor=tk.NW, expand=True, padx=5, pady=5, side="left"
        )
    ...
    def show_comparison_buttons(self):
        ...
        if self.compare_buttons_frame is None:
            ...
            self.abs_diff_button = ttk.Button(
                self.compare_buttons_frame,
                text="Compare Absolute Difference",
                command=self.compare_absolute_difference,
            )
            self.rel_diff_button = ttk.Button(
                self.compare_buttons_frame,
                text="Compare Relative Difference",
    ...
    def compare_absolute_difference(self):
        ...
        if not (
            isinstance(
                f1, (np.ndarray, typing.Iterable, typing.Collection, list, tuple)
            )
            and isinstance(
                f2, (np.ndarray, typing.Iterable, typing.Collection, list, tuple)
            )
        ):
    ...
    def load_file(self, file_path):
        ...
        try:
            ...
            if file_path.suffix == ".npy":
                ...
                msgbox.showerror(
                    message="Unsupported file type. Only .npy and .npz are allowed."
                )
                return None
    ...
    def _load_tree_view(self):
        ...
        cc = self.treeView.insert(
            "",
            0,
            results_dir.name,
            text=results_dir.name,
            values=(
                "Directory",
                stats.st_size,
        ...
        k: "Generator[pathlib.Path]" = walkDirectory(
            results_dir, extension=[".png", ".npz", ".npy"]
        )
        for dtree in k:
            for i in dtree:
                store = False
                    ...
                    if store:
                        ...
                        if not self.treeView.exists(itemname):
                            if part.is_dir():
                                ...
                                part_type = "File"
                            stats = part.stat()
                            currentindex = self.treeView.insert(
                                currentindex,
                                "end",
                                iid=itemname,
                                text=part.name,
                ...
                currentindex = self.treeView.insert(
                    currentindex,
                    "end",
                    iid=i.absolute().resolve().as_posix(),
                    text=i.stem,
    ...
    def create_tree(self):
        ...
        self.treeView.pack(
            expand=True, fill=tk.BOTH, side=tk.TOP, anchor=tk.NW, padx=5, pady=5
        )
        self.treeView.bind("<<TreeviewSelect>>", self._on_tree_item_selected, add="+")

        self.reloadButton = ttk.Button(
            self.treeFrame, text="Reload", command=self._reload_tree_view
        )
        self.newDirectoryButton = ttk.Button(
            self.treeFrame, text="Change Directory", command=self._pick_directory
        )
        self.chooseDataButton = ttk.Button(
            self.treeFrame, text="Choose Data", command=self._choose_data
        )
        self.clearFiles = ttk.Button(
            self.treeFrame, text="Clear Selection", command=self._clear_selection
        )
        self.errorFinderButton = ttk.Button(
