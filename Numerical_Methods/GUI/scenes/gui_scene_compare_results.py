...
def scale_to_fit(a1, a2, copya1=True, copya2=True):
    ...
    if f1.shape != f2.shape:
        ...
        f1slices = [slice(0, wantedShapef1[i], 1) if (
            (i % 2) == 0) else None for i in range(2*f1.ndim)]
        f2slices = [slice(0, wantedShapef2[i], 1) if (
            (i % 2) == 0) else None for i in range(2*f2.ndim)]
        ...
        f1 = np.broadcast_to(
            f1[*f1slices], wantedShapef1).reshape(tuple(finalshape1))
        f2 = np.broadcast_to(
            f2[*f2slices], wantedShapef2).reshape(tuple(finalshape2))
...
#                print('f1',newindex)
                f1 = f1[*newindex].reshape(tuple(f1.shape[z] if z != i else (
                    f1.shape[i]+finalslice1[i]) for z in range(len(f1.shape))))
                ...
                for z in range(2*len(f2.shape)):
                    if (z % 2) == 0:
                        ...
                        if k != i:
                            ...
                            newindex += [(tuple(int(j//2) for j in range(finalslice2[i]))+tuple(range(f2.shape[k]))[
                                          int(finalslice2[i]//2):] + tuple(int(j//2) for j in range(finalslice2[i]))), None]

                f2 = f2[*newindex].reshape(tuple(f2.shape[z] if z != i else (
                    f2.shape[i]+finalslice2[i]) for z in range(len(f2.shape))))
...
def scale_to_scale(a1, a2, copya1=True, copya2=True):
    ...
    if f1.shape != f2.shape:
        ...
        print('finalShape1', finalshape1)
        f1 = np.broadcast_to(
            f1[*f1slices], wantedShapef1).reshape(tuple(finalshape1))[*finalslice]
        f2 = np.broadcast_to(
            f2[*f2slices], wantedShapef2).reshape(tuple(finalshape2))[*finalslice]
    ...
    if f1.shape != f2.shape:
        ...
        for i in range(f1.ndim):
            ...
            if f1.shape[i] < f2.shape[i]:  # Expand f1 by repeating cyclically
                repeats = f2.shape[i] // f1.shape[i]  # Full repeats
                remainder = f2.shape[i] % f1.shape[i]  # Leftover elements
                f1 = np.tile(f1, (repeats + (1 if remainder else 0),
                                  ) + (1,) * (f1.ndim - i - 1))
                ...
                f2 = np.tile(f2, (repeats + (1 if remainder else 0),
                                  ) + (1,) * (f2.ndim - i - 1))
...
class CompareScene(ttk.Frame):
    ...
    def __init__(self, master=None, *args, **kwargs):
        ...
        self.treeFrame.pack(fill=tk.BOTH, anchor=tk.NW,
                            expand=True, padx=5, pady=5, side='left')
        self.treeFrame.propagate(True)

        self.ToggleFrame = ttk.Frame(self)
        self.ToggleFrame.pack(fill=tk.BOTH, side='right', anchor=tk.NW,
                              expand=True, padx=5, pady=5, before=self.treeFrame)
        ...
        self.value_text = tk.StringVar(
            self, "Select Some files to compare them!")
        self.value_label = ttk.Label(
            self.main_frame, textvariable=self.value_text)
...
#         print(file_path)
    ...
    def _load_tree_view(self):
        ...
        cc = self.treeView.insert('', 0, results_dir.name, text=results_dir.name, values=(
            'Directory', stats.st_size, time.ctime(stats.st_mtime), str(results_dir)))
    ...
    def create_tree(self):
        ...
        self.treeView = ttk.Treeview(self.treeFrame, columns=(
            'Type', 'Size', 'Age', "Path"), selectmode=tk.EXTENDED)
        ...
        self.treeView.pack(expand=True, fill=tk.BOTH,
                           side=tk.TOP, anchor=tk.NW, padx=5, pady=5)
        self.treeView.bind('<<TreeviewSelect>>',
                           self._on_tree_item_selected, add='+')
